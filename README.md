...
1. 使用 iOS 16.4 以上的 Safari 開啟 HTTPS 測試網址中的 `water.html`。
2. 點 Safari 分享按鈕，選擇「加入主畫面」。
3. 從主畫面開啟「EIMP 水色」，在「水色異常訊息」按「測試通知」並允許通知。
4. 五秒後會收到「水色辨識異常」系統通知；點擊通知會回到對應案件與地圖。

若曾拒絕通知，請到 iPhone「設定 → 通知 → EIMP 水色」重新開啟。實際背景推播需由後端儲存 Push API 訂閱並發送 Web Push；目前按鈕提供實機系統通知效果測試。
